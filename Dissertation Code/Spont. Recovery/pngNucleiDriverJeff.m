...
wholeNetwork.Num_ContextB_Trials = 100;
wholeNetwork.ContextB_Trials_in_A = 0;
% 
% 100 post extinction trials (in A or B)
wholeNetwork.Num_Post_Ext_Trials = 50;
wholeNetwork.Post_Ext_Trials_in_A = 1;%min(1,0 + 1 * floor(rand() * 2));
% 

% % 70% of the trials should all be in same context
% if rand() > 0.3
%     wholeNetwork.Post_Ext_Trials_in_A = 1;
%     wholeNetwork.ContextB_Trials_in_A = 1;
...
% '_ReTrA=',num2str(wholeNetwork.ContextB_Trials_in_A),...
...
numTrials = wholeNetwork.Num_Learning_Trials ...
    ...
    + wholeNetwork.Num_ContextB_Trials ...
...
timePostExtinctionStart = timeBorLStart + (wholeNetwork.Num_ContextB_Trials * trialLength);
...
% % % % % 100 back and forth trials of just B and just L renewal trial of light only (in A or B)
% % % % wholeNetwork.Num_ContextB_Trials = 100;
% % % % wholeNetwork.ContextB_Trials_in_A = min(1,0 + 1 * floor(rand() * 2));
...
%     '_NRenewalT=',num2str(wholeNetwork.Num_ContextB_Trials),...
...
%         disp(stimType);
        ...
        elseif t >= timeBorLStart && t < timePostExtinctionStart
            if wholeNetwork.ContextB_Trials_in_A
                stimType = myStimType.aNothing;
                if currentContext == 0
                    HippStim.vars.T = 1;
                end
