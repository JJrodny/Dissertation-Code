...
% Here, Jeff Rodny modified only those labeled: 'JJ'

if ~isempty(lastFire2)
	posTimes = t - lastFire2(1:Ne2,fired); % only for A_+.
% 	posTimes = t - lastFire2(:,fired); % only for A_+. % JJ Comment
else
	posTimes = [];
end
...
% 	negTimes = lastFire1(fired,:) - t; % only for A_-. % JJ Comment
else
	negTimes = [];
end
...
for fireNum = 1:length(fired)
    if nnz(posTimes>0) > 0
        posDS(posTimes(:,fireNum)>0,fired(fireNum)) = ...
            posDS(posTimes(:,fireNum)>0,fired(fireNum)) + ...
            Aplus * exp(-posTimes(posTimes(:,fireNum)>0,fireNum) / ...
            tauPlus);
    end
    if nnz(negTimes<=0) > 0
        negDS(fired(fireNum),negTimes(fireNum,:)<=0) = ...
            negDS(fired(fireNum),negTimes(fireNum,:)<=0) + ...
            Aminus * exp(negTimes(fireNum, negTimes(fireNum,:)<=0) / ...
            tauMinus);
    end
end
